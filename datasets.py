...
def build_transform(is_train, args, img_size=224,
    ...
    if is_train:
        ...
        t_list.append(build_customerised_transform(transforms.RandomAffine(0, translate=(0.5, 0.5)),img_size=img_size,mean=mean,std=std))
        t_list.append(build_customerised_transform(transforms.RandomPerspective(distortion_scale=0.5, p=0.5, interpolation=2, fill=0),img_size=img_size,mean=mean,std=std))
        # 增加白噪音
        t_list.append(build_customerised_transform(addPepperNoise.AddPepperNoise(0.9, p=0.5),img_size=img_size,mean=mean,std=std))
